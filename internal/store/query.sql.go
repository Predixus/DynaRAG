...
const searchDocumentsBM25 = `-- name: SearchDocumentsBM25 :many
SELECT 
    d.id,
    d.file_path,
    d.total_chunk_size,
...
type SearchDocumentsBM25Row struct {
	ID             int64
	FilePath       string
...
func (q *Queries) SearchDocumentsBM25(ctx context.Context, toTsquery string) ([]SearchDocumentsBM25Row, error) {
	rows, err := q.db.Query(ctx, searchDocumentsBM25, toTsquery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchDocumentsBM25Row
	for rows.Next() {
		var i SearchDocumentsBM25Row
		if err := rows.Scan(
			&i.ID,
			&i.FilePath,
			&i.TotalChunkSize,
